...
## 可推动木凳：Sprite2D 负责画面，隐藏 CharacterBody3D 在 X/Z 地面处理碰撞。
...
@export var target_position := Vector3(3.1, 0.0, 3.15)
...
func _sync_target_from_marker() -> void:
	...
	if marker == null or not marker.has_method(&"get_logical_position"):
		push_warning("[PushableStool] 目标标记没有逻辑坐标：%s" % target_marker_path)
		return
